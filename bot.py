...
START - تم إزالة التحقق من الاشتراك
...
RECEIVE FILE - تم إزالة التحقق من الاشتراك
...
async def receive_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if video:
        if size > MAX_VIDEO_SIZE:
            await update.message.reply_text("❌ أقصى حجم للفيديو 300MB")
            return
        keyboard = [
            [InlineKeyboardButton("📉 ضغط قوي", callback_data="video_low")],
            [InlineKeyboardButton("⚖️ ضغط متوسط", callback_data="video_medium")],
    ...
    if ext.endswith(".pdf"):
        keyboard = [
            [InlineKeyboardButton("📄 ضغط PDF", callback_data="pdf")]
        ]
        await update.message.reply_text(
            "📄 اختر العملية:",
            reply_markup=InlineKeyboardMarkup(keyboard)
    ...
    if any(ext.endswith(a) for a in archives):
        if size > MAX_ARCHIVE_SIZE:
            await update.message.reply_text("❌ الملف المضغوط أكبر من الحد.")
            return
        keyboard = [
            [InlineKeyboardButton("📦 فك الضغط", callback_data="extract")]
        ]
        await update.message.reply_text(
            "📦 اختر العملية:",
            reply_markup=InlineKeyboardMarkup(keyboard)
...
async def process(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    input_path = os.path.join(TEMP_DIR, data["file_name"])
    ...
    try:
        ...
        if query.data.startswith("video"):
            output = os.path.join(TEMP_DIR, f"compressed_{int(time.time())}.mp4")
            if query.data == "video_low":
                ...
                crf = "22"
            await query.edit_message_text("🎬 جاري ضغط الفيديو...")
            command = [
                "ffmpeg", "-i", input_path,
                "-vcodec", "libx264",
                "-crf", crf,
...
"""
        ...
        # =================================================
            ...
            output = os.path.join(TEMP_DIR, f"compressed_{int(time.time())}.pdf")
...
"""
        ...
        # =================================================
            ...
            await query.edit_message_text("📦 جاري فك الضغط...")
            if ext == ".zip":
            ...
            output_zip = os.path.join(TEMP_DIR, f"extracted_{int(time.time())}.zip")
...
"""
        ...
        await query.edit_message_text("✅ اكتملت العملية")
